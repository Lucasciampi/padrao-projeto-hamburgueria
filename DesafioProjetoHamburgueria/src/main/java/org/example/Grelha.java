...
public class Grelha {
    ...
    public void adicionarCarne(Lanche lanche, String nomeCarne, String nomePonto) {
        PontoCarne ponto = PontoCarneFactory.getPontoCarne(nomePonto);
        Class classe = null;
        try {
            classe = Class.forName("org.example." + nomeCarne);
            Carne carne = (Carne) classe.getDeclaredConstructor(Lanche.class, PontoCarne.class)
                    .newInstance(lanche, ponto);
            carnes.add(carne);
        } catch (Exception e) {
